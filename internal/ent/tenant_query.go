...
func (_q *TenantQuery) QueryMemberships() *TenantMembershipQuery {
	query := (&TenantMembershipClient{config: _q.config}).Query()
...
func (_q *TenantQuery) WithMemberships(opts ...func(*TenantMembershipQuery)) *TenantQuery {
	query := (&TenantMembershipClient{config: _q.config}).Query()
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := _q.withMemberships; query != nil {
		if err := _q.loadMemberships(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Memberships = []*TenantMembership{} },
			func(n *Tenant, e *TenantMembership) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TenantQuery) loadMemberships(ctx context.Context, query *TenantMembershipQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantMembership)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantmembership.FieldTenantID)
	}
