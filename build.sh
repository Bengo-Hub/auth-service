...
SERVICE_DB_NAME=${SERVICE_DB_NAME:-auth}
SERVICE_DB_USER=${SERVICE_DB_USER:-auth_user}
...
if ! kubectl -n "$NAMESPACE" get secret "$ENV_SECRET_NAME" >/dev/null 2>&1; then
  ...
  kubectl -n "$NAMESPACE" create secret generic "$ENV_SECRET_NAME" \
    --from-literal=AUTH_DB_URL="postgresql://${SERVICE_DB_USER}:PASSWORD@postgresql.infra.svc.cluster.local:5432/${SERVICE_DB_NAME}?sslmode=disable" \
    --from-literal=AUTH_REDIS_ADDR="redis-master.infra.svc.cluster.local:6379" || true
fi
